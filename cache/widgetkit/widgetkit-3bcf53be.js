...
function wk_ajax_render_url(widgetid) {
	return"/component/widgetkit/?tmpl=raw&amp;id=" + widgetid
}
(function (f, e) {
	var a = {};
	e.$widgetkit = {lazyloaders:{}, load:function (b) {
		a[b] || (a[b] = f.getScript(b));
		return a[b]
	}, lazyload:function (a) {
		f("[data-widgetkit]", a || document).each(function () {
			var a = f(this), b = a.data("widgetkit"), e = a.data("options") || {};
			!a.data("wk-loaded") && $widgetkit.lazyloaders[b] && ($widgetkit.lazyloaders[b](a, e), a.data("wk-loaded", true))
		})
	}};
	f(function () {
		$widgetkit.lazyload()
...
(function (f, e) {
	e.$widgetkit.trans = {__data:{}, addDic:function (a) {
		f.extend(this.__data, a)
	}, add:function (a, b) {
		this.__data[a] = b
	}, get:function (a) {
		if (!this.__data[a])return a;
		var b = arguments.length == 1 ? [] : Array.prototype.slice.call(arguments, 1);
...
(function (f) {
	...
	f.extend(f.easing, {def:"easeOutQuad", swing:function (e, a, b, c, d) {
		...
		if ((a /= d) == 1)return b + c;
		g || (g = d * 0.3);
		f < Math.abs(c) ? (f = c, e = g / 4) : e = g / (2 * Math.PI) * Math.asin(c / f);
		return-(f * Math.pow(2, 10 * (a -= 1)) * Math.sin((a * d - e) * 2 * Math.PI / g)) + b
		...
		g || (g = d * 0.3);
		f < Math.abs(c) ? (f = c, e = g / 4) : e = g / (2 * Math.PI) * Math.asin(c / f);
		return f * Math.pow(2, -10 * a) * Math.sin((a * d - e) * 2 * Math.PI / g) + c + b
		...
		g || (g = d * 0.3 * 1.5);
		f < Math.abs(c) ? (f = c, e = g / 4) : e = g / (2 * Math.PI) * Math.asin(c / f);
		return a < 1 ? -0.5 * f * Math.pow(2, 10 * (a -= 1)) * Math.sin((a * d - e) * 2 * Math.PI / g) + b : f * Math.pow(2, -10 * (a -= 1)) * Math.sin((a * d - e) * 2 * Math.PI / g) * 0.5 + c + b
		...
		f == void 0 && (f = 1.70158);
		return(a /= d / 2) < 1 ? c / 2 * a * a * (((f *= 1.525) + 1) * a - f) + b : c / 2 * ((a -= 2) * a * (((f *= 1.525) + 1) * a + f) + 2) + b
	}, easeInBounce:function (e, a, b, c, d) {
		return c - f.easing.easeOutBounce(e, d - a, 0, c, d) + b
		...
		return a < d / 2 ? f.easing.easeInBounce(e, a * 2, 0, c, d) * 0.5 + b : f.easing.easeOutBounce(e, a * 2 - d, 0, c, d) * 0.5 + c * 0.5 + b
	}})
})(jQuery);
(function (f) {
	function e(a) {
		var c = a || window.event, d = [].slice.call(arguments, 1), e = 0, i = 0, h = 0, a = f.event.fix(c);
		a.type = "mousewheel";
		a.wheelDelta && (e = a.wheelDelta / 120);
		a.detail && (e = -a.detail / 3);
		h = e;
		c.axis !== void 0 && c.axis === c.HORIZONTAL_AXIS && (h = 0, i = -1 * e);
		c.wheelDeltaY !== void 0 && (h = c.wheelDeltaY / 120);
		c.wheelDeltaX !== void 0 && (i = -1 * c.wheelDeltaX / 120);
		d.unshift(a, e, i, h);
	...
	f.event.special.mousewheel = {setup:function () {
		if (this.addEventListener)for (var b = a.length; b;)this.addEventListener(a[--b], e, false); else this.onmousewheel = e
	}, teardown:function () {
		if (this.removeEventListener)for (var b = a.length; b;)this.removeEventListener(a[--b], e, false); else this.onmousewheel = null
	}};
...
(function (d) {
	...
	a.prototype = d.extend(a.prototype, {name:"accordion", options:{index:0, duration:500, easing:"easeOutQuart", animated:"slide", event:"click", collapseall:!0, matchheight:!0, toggler:".toggler", content:".content"}, initialize:function (a, b) {
		...
		c.each(function (a) {
			var c = d(this), e = c.next().css("overflow", "hidden");
			e.data("height", e.height()).addClass("content-wrapper");
			a == b.index || b.index == "all" ? (c.addClass("active"), e.show()) : e.hide().css("height", 0);
...
if (!window['mejs']) {
	$widgetkit.load('/media/widgetkit/widgets/mediaplayer/mediaelement/mediaelement-and-player.js').done(function () {
		jQuery(function ($) {
			...
			$('video,audio').mediaelementplayer({"pluginPath":"\/media\/widgetkit\/widgets\/mediaplayer\/mediaelement\/"});
		});
	});
} else {
	jQuery(function ($) {
...
(function (d) {
	$widgetkit.lazyloaders.slideset = function (a, b) {
		var c = a.find("ul.set").show();
		gwidth = b.width == "auto" ? a.width() : b.width;
		d.browser.msie && d.browser.version < 8 && c.children().css("display", "inline");
		var e = b.height == "auto" ? c.eq(0).outerHeight(!0) : b.height;
		c.eq(0).parent().css({height:e});
		a.css({width:gwidth});
		c.css({height:e});
		$widgetkit.load(WIDGETKIT_URL + "/widgets/slideset/js/slideset.js").done(function () {
			a.slideset(b).css("visibility", "visible");
			a.find("img[data-src]").each(function () {
				var a = d(this), b = a.data("src");
				setTimeout(function () {
					a.attr("src", b)
				}, 1)
			})
...
(function (c) {
	$widgetkit.lazyloaders.slideshow = function (b, a) {
		b.css("visibility", "hidden");
		var d = 0, f = 0, g = b.find("ul.slides:first");
		g.children().each(function () {
			d = Math.max(d, c(this).height());
			f = Math.max(f, c(this).width())
		});
		if (a.height == "auto")a.height = d;
		...
		$widgetkit.load(WIDGETKIT_URL + "/widgets/slideshow/js/slideshow.js").done(function () {
			...
			b.find("img[data-src]").each(function () {
				var a = c(this), b = a.data("src");
				setTimeout(function () {
					a.attr("src", b)
				}, 1)
			})
	...
	$widgetkit.lazyloaders.showcase = function (b, a) {
		var d = b.find(".wk-slideshow").css("visibility", "hidden"), f = b.find(".wk-slideset").css("visibility", "hidden"), g = f.find("ul.set > li"), h = 0, i = 0, e = b.find("ul.slides:first");
		e.children().each(function () {
			h = Math.max(h, c(this).height());
			i = Math.max(i, c(this).width())
		});
		if (a.height == "auto")a.height = h;
		if (a.width == "auto")a.width = b.width();
		d.css({position:"relative", width:a.width});
		e.css({position:"relative", overflow:"hidden", height:a.height}).children().css({top:"0px", left:"0px", position:"absolute", width:b.width(), height:a.height});
		e = b.find("ul.set");
		gwidth = a.width == "auto" ? b.width() : a.width;
		c.browser.msie && c.browser.version < 8 && e.children().css("display", "inline");
		var j = e.eq(0).outerHeight(!0);
		b.css({width:gwidth});
		e.css({height:j}).hide();
		f.css("height", f.height() + j);
		e.show();
		c.when($widgetkit.load(WIDGETKIT_URL + "/widgets/slideset/js/slideset.js"), $widgetkit.load(WIDGETKIT_URL + "/widgets/slideshow/js/slideshow.js")).done(function () {
			d.slideshow(a).css("visibility", "visible");
			...
			var b = d.data("slideshow"), e = f.data("slideset");
			g.eq(b.index).addClass("active");
			d.bind("slideshow-show", function (a, b, c) {
				if (!g.removeClass("active").eq(c).addClass("active").parent().is(":visible"))e[c > b ? "next" : "previous"]()
			});
			g.each(function (a) {
...
jQuery(function (b) {
	var f = function (b) {
		var a = new Date(Date.parse(b.replace(/(\d+)-(\d+)-(\d+)T(.+)([-\+]\d+):(\d+)/g, "$1/$2/$3 $4 UTC$5$6"))), a = parseInt(((arguments.length > 1 ? arguments[1] : new Date).getTime() - a) / 1E3);
		return a < 60 ? $widgetkit.trans.get("LESS_THAN_A_MINUTE_AGO") : a < 120 ? $widgetkit.trans.get("ABOUT_A_MINUTE_AGO") : a < 2700 ? $widgetkit.trans.get("X_MINUTES_AGO", parseInt(a / 60).toString()) : a < 5400 ? $widgetkit.trans.get("ABOUT_AN_HOUR_AGO") : a < 86400 ? $widgetkit.trans.get("X_HOURS_AGO", parseInt(a / 3600).toString()) : a < 172800 ? $widgetkit.trans.get("ONE_DAY_AGO") : $widgetkit.trans.get("X_DAYS_AGO", parseInt(a / 86400).toString())
	};
